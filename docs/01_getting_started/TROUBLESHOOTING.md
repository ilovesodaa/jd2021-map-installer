...
- **JDNext mapPackage extraction fails with `AssetStudioModCLI.exe not found under 3rdPartyTools`** — The installer now resolves this binary only from local `3rdPartyTools` paths. Stage the runtime bundle under `3rdPartyTools/Unity2UbiArt/bin/AssetStudioModCLI/` and retry.
